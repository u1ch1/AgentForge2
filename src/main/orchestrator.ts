...
- для backend выбирай стек по задаче, а не только Express по умолчанию:
  ...
  поддержаны проверкой сборки и «Просмотром». WordPress (тема или плагин)
  тоже поддержан, но иначе: живого WordPress+MySQL для сборки и запуска
  здесь нет, поэтому «Просмотра» и рантайм-проверки для него не будет —
  только код по правилам темы/плагина и ревью Тестера;
